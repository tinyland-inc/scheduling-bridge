...
Small-business SaaS platforms create vendor lock-in through API paywalls and proprietary data formats. We present a production middleware that functions as an anti-corruption layer, implementing a 17-method scheduling adapter interface against vendors that block programmatic access -- driving the vendor's public-facing web UI via headless browser automation where a REST API is paywalled (replacing 8 blocked REST endpoints through DOM interaction alone), and falling through to a vendor's own REST API where one is available. The contribution this paper reports is an engineering one: we model each booking flow as a typed effect program and derive a serializable flow DAG (a `FlowPlan` with a `sha256` plan hash) as a *projection* of the very definitions that execute, rather than as a second, hand-maintained source of truth. A single execution combinator -- one fold over the derived plan -- replaced three divergent hand-written booking compositions whose page-lifecycle scoping had quietly drifted apart; the divergence is now structurally inexpressible. To migrate without trusting the rewrite, we recorded golden step traces from the three legacy compositions before deleting them, then made a conformance harness assert the fold reproduces those traces byte-for-byte across happy and failure paths -- an anti-renaming guarantee that the new path is behaviorally the old one, not merely a renamed copy. Because each flow carries its own typed effect-requirement (`R`) channel, transport-agnosticism is a compile-time property, not a hope: a read-only CalCom REST adapter reuses the identical availability flow over an HTTP client, and a source-level type fence proves its steps never demand a browser service or browser scope -- so a REST flow provisions no browser layer. The system has been deployed in production since March 2026 (now on a self-hosted Kubernetes cluster), and we ground the work in lessons learned from automating 604 legacy appointments across 62 weeks of calendar data, the migration that collapsed the dual code paths to one, and the reusable FOSS offramp the adapter hub gives small businesses escaping proprietary scheduling services.
