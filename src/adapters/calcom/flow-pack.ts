 ...
 *
 * `matchers.service` reuses the SHARED `makeServiceMatcher` machinery (fuzzy scorers are
 * vendor-neutral pure functions) so CalCom event-type matching scores identically to
 * Acuity service matching. `paymentInjection: 'native'` — CalCom takes payment natively
